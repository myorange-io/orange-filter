...
사이드 패널을 처음 열면 안내 화면이 표시되며 한국어 정밀 보호 모델(173.7MB)을 1회 다운로드합니다 — 이후 인터넷 없이도 동작합니다. 모델이 설치되기 전까지는 입력 UI 자체를 차단합니다. 잘못된 안심을 주지 않기 위함입니다.
...
- **사용자 데이터 외부 전송 없음** — 입력한 텍스트와 파일은 PC를 떠나지 않습니다. 확장이 외부로 만드는 통신은 사용자 정보와 무관한 두 가지뿐입니다 — (1) 모델 가중치 1회 다운로드, (2) GitHub repo에서 받는 오탐 차단 목록 갱신(24시간 주기, GET만). 양쪽 다 사용자 정보를 포함하지 않습니다.
- **분리된 미리보기 영역** — 마스킹 미리보기 창은 ChatGPT·Claude 같은 사이트가 들여다볼 수 없는 영역에 떠 있어, 사이트 코드가 마스킹 전 원문을 가로챌 수 없습니다.
- **최소 권한** — Chrome에 요구하는 권한은 설정 저장·사이드 패널 표시·모델 실행 3가지뿐입니다. 브라우저 탭·쿠키·웹 트래픽 가로채기 권한은 요구하지 않습니다.
...
- **온디바이스 AI** — 한국어 정밀 보호 모델(173.7MB)을 한 번 설치하면 인터넷 없이도 동작합니다. 자연 문장 안의 사람 이름·주소처럼 패턴만으로는 못 잡는 식별자를 컨텍스트로 판단합니다.
