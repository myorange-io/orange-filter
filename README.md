# Orange Filter v1.1.0 소개

LLM에 텍스트를 붙여넣거나 파일을 올리기 전에, 사용자의 PC 안에서 개인정보를 가려내고 마스킹하는 Chrome 확장을 소개합니다.

---

오늘 우리는 한국 비영리(NPO) 현장의 개인정보 보호 격차를 메우기 위한 도구를 공개합니다. Orange Filter는 ChatGPT·Claude·Gemini·Perplexity·오렌지임팩트 등 LLM 인터페이스 앞단에서 동작하며, 결산공시·기부금 명세서 같은 정형 양식이 외부 모델에 노출되기 전에 17개 카테고리의 개인 식별 정보(PII)를 탐지·마스킹합니다.
...
검토 통과 후 Chrome 웹 스토어에서 "Orange Filter" 검색 후 설치할 수 있습니다. 사이드 패널을 한 번 열어 한국어 정밀 보호 모델(약 50MB)을 다운로드하면 이후 인터넷 없이도 동작합니다.
