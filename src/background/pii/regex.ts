...
// 존칭 없이도 매칭하지만 confidence 낮음
const NAME_BARE = new RegExp(
  `(?<![가-힣])${SURNAME_CLASS}[가-힣]{1,2}(?![가-힣])`,
  'g',
);
...
function detectKoreanName(text: string): RawMatch[] {
  ...
  // bare 이름 — 낮은 confidence (모델 검증 권장)
  // FP 위험 큼: 일반 명사 (김치/이순신/박물관 등)와 충돌. v1에서는 confidence만 표시.
  for (const m of text.matchAll(NAME_BARE)) {
    if (m.index === undefined) continue;
    // 이미 NAME_WITH_TITLE이 잡은 영역과 겹치면 skip (병합 단계에서 처리하지만 여기선 단순 dedupe)
    out.push({
      start: m.index,
      end: m.index + m[0].length,
      text: m[0],
