...
describe('detectDate — YYYY[.-/]M[.-/]D + 한국어', () => {
  it('YYYY.M.D (생년월일 형식)', () => {
    const spans = detectKoreanPII('1983.8.31');
    expect(spans.find((s) => s.category === 'date')?.text).toBe('1983.8.31');
  });
  ...
  it('한국어 1983년 8월 31일', () => {
    const spans = detectKoreanPII('1983년 8월 31일');
    expect(spans.find((s) => s.category === 'date')?.text).toBe('1983년 8월 31일');
  });
...
describe('detectAddress — 한국 주소', () => {
  it('서울 영등포구 양평로28길 5 매칭', () => {
    const spans = detectKoreanPII('서울 영등포구 양평로28길 5');
    const addr = spans.find((s) => s.category === 'address');
    expect(addr?.text).toContain('서울 영등포구');
  });

  it('번지 + 동호수까지 매칭 (5, 103-802)', () => {
    const spans = detectKoreanPII('서울 영등포구 양평로28길 5, 103-802');
    const addr = spans.find((s) => s.category === 'address');
    expect(addr?.text).toBe('서울 영등포구 양평로28길 5, 103-802');
  });

  it('주소 다음 줄로 새지 않음 (개행 차단)', () => {
    const spans = detectKoreanPII('서울 영등포구 양평로28길 5\n다음 줄 텍스트');
    const addr = spans.find((s) => s.category === 'address');
    expect(addr?.text).toBe('서울 영등포구 양평로28길 5');
  });
